...
app.post("/webhook", async (req, res) => {
 ...
 var licValid = await validateWhopLicense();
 if (!licValid) return res.status(403).json({ error: "Invalid or expired license. Visit whop.com to manage your subscription." });
 if (!rh.getToken()) {
   var ok = await ensureLoggedIn();
